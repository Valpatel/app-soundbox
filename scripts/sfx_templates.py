"""
SFX Templates for Bulk Generation

Clean, isolated sound effect prompts organized by category.
All prompts designed to produce:
- Isolated sounds (no background noise)
- Dry recordings (no reverb)
- Single sound events (not layered)
- Studio quality output
"""

import random

# Category distribution for generation targets
CATEGORY_DISTRIBUTION = {
    # Animals - common sound needs
    "bird": 75,
    "dog": 75,
    "cat": 60,
    "insect": 60,

    # Trailer & Cinematic - heavily used
    "riser": 100,
    "boom": 100,
    "stinger": 100,
    "swell": 80,
...
def get_all_sfx_prompts() -> list:
    """
    Get all SFX prompts formatted for the bulk generator.
    Returns list of dicts with 'text' and 'category' keys.
    """
    all_prompts = []
    ...
    for category, target_count in CATEGORY_DISTRIBUTION.items():
        ...
        # Generate enough prompts to reach target
        for i in range(target_count):
            prompt = base_prompts[i % len(base_prompts)]

            # Add variation for repeated prompts
            if i >= len(base_prompts):
                variation = (i // len(base_prompts)) + 1
...
def get_category_stats() -> dict:
    ...
    for category, target in CATEGORY_DISTRIBUTION.items():
        base_count = len(PROMPTS.get(category, []))
        stats[category] = {
            'base_prompts': base_count,
            'target': target,
        }
    return stats
...
if __name__ == "__main__":
    print("=" * 60)
    print("SFX TEMPLATES SUMMARY")
    print("=" * 60)

    total_prompts = 0
    total_target = 0

    for category, target in sorted(CATEGORY_DISTRIBUTION.items()):
        base = len(PROMPTS.get(category, []))
        total_prompts += base
        total_target += target
        print(f"{category:15s}: {base:3d} base prompts -> {target:3d} target clips")

    print("=" * 60)
    print(f"Total: {len(CATEGORY_DISTRIBUTION)} categories")
    print(f"       {total_prompts} base prompts")
    print(f"       {total_target} target clips")
